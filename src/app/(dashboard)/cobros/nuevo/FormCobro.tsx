...
interface Cuenta {
  ...
  nombre: string
  saldo: number
  cliente: { id: string; nombreRazonSocial: string } | null
}
...
export function FormCobro({ cuentas, preseleccionId }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
          ...
          <div className="border rounded-md divide-y max-h-52 overflow-y-auto bg-background shadow-sm">
            {cuentasFiltradas.length === 0 ? (
              ...
              cuentasFiltradas.map((c) => (
                ...
                >
                  <span className="font-medium">{c.cliente?.nombreRazonSocial ?? c.nombre}</span>
                  <span className={cn(
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold">{cuentaSeleccionada.cliente?.nombreRazonSocial ?? cuentaSeleccionada.nombre}</p>
                ...
                <div className="text-right">
                  ...
                  {cuentaSeleccionada.saldo > 0 && (
                    <Badge variant="destructive" className="text-xs mt-1">Debe al negocio</Badge>
                  )}
                  {cuentaSeleccionada.saldo <= 0 && (
      ...
      <div className="space-y-2">
        ...
        <Input
          id="concepto"
          placeholder="Ej: Pago factura, Abono parcial..."
          value={concepto}
